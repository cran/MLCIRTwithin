est_multi_glob_gen <- function (Y, X, model, ind = 1:nrow(Y), be = NULL, Dis = NULL, dis = NULL, 
                            disp=FALSE, only_sc = FALSE, Int = NULL, der_single = FALSE,maxit=10){
                            	
# *** to be used when the number of categories is different between the rows of Y ***
        	...
        	num = den = 0
   	        for (h in 1:nd){
	            ...
	            if(sum(indh)==1){
	            	...
	            	dist = colSums(YW[indh,1:lv[h]],na.rm=TRUE)+0.5
	            }
...
#		        be = be+ginv(den)%*%num*sum(w[indh])/sum(w)
    ...
    for (h in 1:nd) {
        Xh = matrix(X[1:(lv[h]-1), , h],lv[h]-1, ncov)
        if(is.null(Int)) x0 = 0 else x0 = Int[1:(lv[h]-1),h]
        if (model == "m") {
            P[h, ] = exp(G %*% (x0+Xh%*%be))
            P[h, ] = P[h, ]/sum(P[h, ])
        }
        ...
        if(!only_sc){
        	...
        	for (h in 1:nd) {
	            ...
	            if (model == "m") 
	        	    ...
	        	    P[h, ] = P[h, ]/sum(P[h, ])
            	if (model == "g" || model == "l") 
                	P[h,1:lv[h]] = inv_glob(x0+X[1:(lv[h]-1),,h]%*%be,model)$p
        	}
        	P = pmax(P, 10^-100)
	    	Pdis = P; P = P[ind,]
        	lko = lk
